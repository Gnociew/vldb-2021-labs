...
func (ps *PeerStorage) Append(entries []eraftpb.Entry, raftWB *engine_util.WriteBatch) error {
	...
	for _, entry := range entries {
		...
		//       Note the raft logs are stored as the `meta` type key-value pairs, so the `RaftLogKey` and `SetMeta`
		//       functions could be useful.
		raftWB.SetMeta(meta.RaftLogKey(ps.region.GetId(), entry.GetIndex()), &entry)
		log.Debug(fmt.Sprintf("entry=%v", entry))
	}
	...
	for i := lastIndex + 1; i <= prevLastIndex; i++ {
		...
		log.Debug(fmt.Sprintf("delete entry index %d", i))
	}
...
func (ps *PeerStorage) SaveReadyState(ready *raft.Ready) (*ApplySnapResult, error) {
	...
	if len(ready.Entries) != 0 {
		...
		if err != nil {
			log.Warn(fmt.Sprintf("err in Append: %v", err))
		}
	}
